...
> Project house style — apply this to every prose artifact you produce in this repo: PR and commit bodies, status updates, handoffs, summaries.
...
- **Terse line** (the default) — one or two plain sentences. *No* headers or bullets; structure on
  ...
  > `All 38 tests pass; ruff flags one fixable issue — fixing and re-running.`

- **Full artifact** (high stakes: a review, recommendation, audit, handoff, PR body) — open with a
  **bolded one-line verdict**, then bolded topic-sentences leading each paragraph, lists or tables
  only where they aid navigation, every claim anchored to a `path`, count, or commit hash.
